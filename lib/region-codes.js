 ...
 * @param {string} regionName - Full name of the region (e.g., 'Alaska')
 * @returns {string|null} - Region code (e.g., 'AK') or null if not found
 */
function getRegionCodeByName(regionName) {
  const region = REGION_CODES.find(([country, code, name]) => name === regionName);
  return region ? region[1] : null; // Return the code or null if not found
}
